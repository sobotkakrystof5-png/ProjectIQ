...
export default function EarningsCalculator({
  ...
  const projectTotal = (p: CompletedProject) => Number(p.amount) + (p.deposit_amount ? Number(p.deposit_amount) : 0)
  const totalEarnings = projects.reduce((s, p) => s + projectTotal(p), 0)
  const clientProjects = projects.filter(p => p.project_type === 'client')
  const clientEarnings = clientProjects.reduce((s, p) => s + projectTotal(p), 0)
  const personalEarnings = projects.filter(p => p.project_type === 'personal').reduce((s, p) => s + projectTotal(p), 0)
